...
# -*- coding: utf-8 -*-
"""
Created on Thu Aug 17 16:52:15 2023

@author: marvazquezrabunal
"""

"""
CROSS VALIDATION SURVIVAL NO TIME


Description:

Code to perform cross validation on survival datasets considering that the 
model does not have time dependence.
...
from skorch_notime_survival import NeuralNet_survival_notime
...
class CrossValidation_notime_survival:
    def __init__(
            self,
            module,
            criterion,
    ...
    def path(self, X_train, y_train, X_test, y_test, lambda_seq, alpha):
        ...
        net_new = NeuralNet_survival_notime(
            module_,
            max_epochs = 40,
    ...
    def proximal_operator(self, non_linear_weights, linear_weights, lambda1, alpha):
        ...
        v2 = self.gl_operator(non_linear_weights, lambda1 * l * (1-alpha)).astype('float32')
...
### Fit final model with optimal hyperparameters for survival model without
### time dependence


class final_model_notime_survival:
    def __init__(
            self,
            module,
            criterion,
