...
# -*- coding: utf-8 -*-
"""
Created on Thu Aug  3 10:05:35 2023
...
"""
SUBCLASS SKORCH SURVIVAL DATA

Description:
    
Modification of the skorch class NeuralNet to include the proximal operator
for survival data.
"""
...
class NeuralNet_survival:
    ...
    lambda1: float (default=0.0)
        Penalization parameter passed to the proximal operator.
    ...
    # I add as default warm_start = True
    ...
    def __init__(
            ...
            warm_start=True,  # I set as default warm_start=True
            verbose=1,
            device='cpu',
            compile=False,
            zeros_lin = [],
        ...
        self.zeros_lin = zeros_lin
        self.zeros_time = zeros_time
    ...
    def validation_step(self, batch, **fit_params):
        ...
        **fit_params : dict
        ...
        with torch.no_grad():
            ...
            loss = self.get_loss(self.module_, y_pred, yi, X=Xi, training=False)
        return {
            'loss': loss,
            'y_pred': y_pred,
        }
    ...
    def train_step_single(self, batch, **fit_params):
        ...
        **fit_params : dict
        ...
        Xi, yi = unpack_data(batch)
        y_pred = self.infer(Xi, **fit_params)
        loss = self.get_loss(self.module_, y_pred, yi, X=Xi, training=True)
        loss.backward()
        return {
            'loss': loss,
            'y_pred': y_pred,
    ...
    def proximal_operator(self, time_weights, non_linear_weights, linear_weights, lambda1):
        ...
        v1 = self.gl_operator(time_weights, l * lambda1 * (1-self.alpha)*(1- self.gamma)).astype('float32')
        v1 = np.concatenate((v1, non_linear_weights))
        v2 = self.gl_operator(v1, l * lambda1 * (1-self.alpha)*(self.gamma)).astype('float32')
        v2 = np.concatenate((v2, linear_weights))
        v3 = self.gl_operator(v2, l * lambda1 * self.alpha).astype('float32')
        return(v3)
    ...
    def _step_optimizer(self, step_fn):
        ...
        for name in self._optimizers:
            ...
            #print(list(self.module_.parameters())[1])
         
            if self.lambda1 != 0:
                with torch.no_grad():
                    for i in range(len(self.module_.linear)):
                        time_weights = np.empty(0)
                        for param in self.module_.non_linear_time[i].parameters():
                            time_weights = np.concatenate((time_weights, param.data.reshape(-1).numpy()))
                        non_linear_weights = np.empty(0)
                        for param in self.module_.non_linear[i].parameters():
                            non_linear_weights = np.concatenate((non_linear_weights, param.data.reshape(-1).numpy()))
                        linear_weights = np.empty(0)
                        for param in self.module_.linear[i].parameters():
                            linear_weights = np.concatenate((linear_weights, param.data.reshape(-1).numpy()))
                        prox_op = self.proximal_operator(time_weights, non_linear_weights, linear_weights, self.lambda1 * self.lr)
                        j = 0
                        for param in self.module_.non_linear_time[i].parameters():
                            shape1 = param.shape
                            num_elem1 = torch.numel(param)
                            values_param = torch.from_numpy(prox_op[j: j + num_elem1])
                            param.data = values_param.reshape(shape1)
                            j = j + num_elem1
                        for param in self.module_.non_linear[i].parameters():
                            shape1 = param.shape
                            num_elem1 = torch.numel(param)
    ...
    # pylint: disable=unused-argument
    def get_loss(self, model, y_pred, y_true, X=None, training=False):
        ...
        return self.criterion_(model, y_pred, y_true, X, training)
