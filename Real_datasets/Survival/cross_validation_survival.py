...
# -*- coding: utf-8 -*-
"""
Created on Thu Aug  3 10:03:22 2023
...
"""
CROSS VALIDATION SURVIVAL


Description:

Code to perform cross validation on survival datasets.
...
# Add the path to import the NeuralNetPO class
from skorch_survival import NeuralNet_survival


###----------------------------------------------------------------------------

### Create the class to perform cross validation

class CrossValidation_survival:
    def __init__(
            self,
            module,
            criterion,
    ...
    def path(self, X_train, y_train, X_test, y_test, lambda_seq, alpha):
        ...
        module_ = self.module(X_train.shape[1]-1)
        valid_ds = Dataset(X_test, y_test)
        net_new = NeuralNet_survival(
            module_,
            max_epochs = 40,
        ...
        for lambda1 in lambda_seq:
            ...
            for i in range(len(net.module_.linear)):
                time_weights = np.empty(0)
                for param in net.module_.non_linear_time[i].parameters():
                    time_weights = np.concatenate((time_weights, param.data.reshape(-1).numpy()))
    ...
    def proximal_operator(self, time_weights, non_linear_weights, linear_weights, lambda1, alpha):
        """Apply proximal operator to a vector.

        Parameters
        ----------
        time_weights: vector of time-dependent weights.
        ...
        """
        l = np.sqrt(len(time_weights) + len(non_linear_weights) + len(linear_weights))
        v1 = self.gl_operator(time_weights, l* lambda1 * (1-alpha)*(1-self.gamma)).astype('float32')
        v1 = np.concatenate((v1, non_linear_weights))
        v2 = self.gl_operator(v1, lambda1 * l * (1-alpha)*(self.gamma)).astype('float32')
        v2 = np.concatenate((v2, linear_weights))
        v3 = self.gl_operator(v2, lambda1 * l * (alpha)).astype('float32')
        return(v3)
    ...
    def obtain_lambda_start(self, X, y, alpha):
        ...
        for lambda1 in lambda_seq:
            for i in range(len(new_net.module_.linear)):
                time_weights = np.empty(0)
                for param in new_net.module_.non_linear_time[i].parameters():
                    time_weights = np.concatenate((time_weights, param.data.reshape(-1).numpy()))
                non_linear_weights = np.empty(0)
                ...
                for param in new_net.module_.linear[i].parameters():
                    linear_weights = np.concatenate((linear_weights, param.data.reshape(-1).numpy()))
                prox_op = self.proximal_operator(time_weights, non_linear_weights, linear_weights, self.lr * lambda1, alpha)
                if abs(prox_op[0: len(time_weights)]).sum() == 0:
...
### Fit final model with optimal hyperparameters for survival model


class final_model_survival:
    def __init__(
            self,
            module,
            criterion,
    ...
    def fit(self, X, y, lambda_opt):
        ...
        module_ = self.module(X.shape[1]-1)
        X_train, X_test, y_train, y_test = train_test_split(X, y, test_size = 0.2)
        valid_ds = Dataset(X_test, y_test)
        es = skorch.callbacks.EarlyStopping()
        ...
        module2 = self.module(X.shape[1]-1)
        ...
        lin = []
        for name, param in net.module_.non_linear.named_parameters():
            if param.data.sum() != 0:
                lin.append([*name][0])
        if len(list(np.unique(lin))) == X_train.shape[1] - 1: 
            return net
        else:
            for i in list(np.unique(lin)):
                non_lin_freez.append('linear.'+i+'.fc1.weight')
            
            module3 = self.module(X.shape[1]-1)
            module3.load_state_dict(net.module_.state_dict())
            
            for name, param in module3.named_parameters():
                if name not in non_lin_freez:
                    # param.data = param.data*random.random()
