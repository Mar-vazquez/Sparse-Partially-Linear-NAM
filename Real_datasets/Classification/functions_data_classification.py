...
# -*- coding: utf-8 -*-
"""
Created on Sun Aug 27 21:05:01 2023
...
"""
FUNCTIONS REAL DATA CLASSIFICATION


Description:

Code with the functions to apply our model to real-world classification datasets,
doing CV, obtaining the optimal hyperparameters and the evaluation metrics. It also contains
...
import torch
import random
from cross_validation_classification import CrossValidation_classification, final_model_classification
...
from model_classification import NAM_classification
...
def fit_model_classif(X_train, y_train, seed):
    ...
    example = CrossValidation_classification(model_classification, max_epochs = 50, criterion = torch.nn.BCELoss(),
...
def final_model(X, y, opt_param, seed):
    ...
    val = final_model_classification(model_classification, max_epochs = 50, criterion = torch.nn.BCELoss, alpha = alpha_opt, lr = lr_opt, optimizer = torch.optim.Adam)
...
def results_data_classif(X_train, y_train, X_test, y_test, seed):
    ...
    Returns
    -------
    List with the predicted probabilities, optimal hyperparameters, structure
    obtained, F1 score and AUC.
    
    """
    model = fit_model_classif(X_train, y_train, seed)
    opt_param = optimal_param(model)
    final_model_classif = final_model(X_train, y_train, opt_param, seed)
    structure = structure_found(X_train.shape[1], final_model_classif)
    pred = final_model_classif.predict(X_test)
    final_pred = (pred[:,0]<pred[:,1])*1
    f1_sco = f1_score(y_test, final_pred)
    auc = roc_auc_score(y_test, pred[:,1])
    return [pred, opt_param, structure, f1_sco, auc]
...
def NAM_fit_classif(X, y):
    ...
    """
    module_ = NAM_classification(X.shape[1])
...
def results_data_NAM_classif(X_train, y_train, X_test, y_test):
    ...
    Returns
    -------
    List with the predicted probabilities, the F1 score and the AUC.
    
    """
    model = NAM_fit_classif(X_train, y_train)
    pred = model.predict(X_test)
    final_pred = (pred[:,0]<pred[:,1])*1
    f1_sco = f1_score(y_test, final_pred)
    auc = roc_auc_score(y_test, pred[:,1])
    return [pred[:,1], f1_sco, auc]
...
def sparse_NAM_classif(X_train, y_train, seed):
...
def results_sparse_NAM_classif(X_train, y_train, X_test, y_test, seed):
    """Do CV, obtain optimal hyperparameters, fit final model of sparse NAM,
    obtain structure of the features and predictive metrics.
    ...
    """
    model = sparse_NAM_classif(X_train, y_train, seed)
    ...
    f1_sco = f1_score(y_test, final_pred)
    auc = roc_auc_score(y_test, pred[:,1])
    return [pred, opt_param_sparse, structure, f1_sco, auc]
