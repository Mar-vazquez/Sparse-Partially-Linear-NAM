...
# -*- coding: utf-8 -*-
"""
Created on Thu Aug 17 16:49:24 2023

@author: marvazquezrabunal
"""

"""
SUBCLASS SKORCH SURVIVAL DATA WITHOUT TIME DEPENDENCE

Description:
    
Modification of the skorch class NeuralNet to include the proximal operator
for survival data in the case in which the model does not have time dependence.
"""

###----------------------------------------------------------------------------

## Subclass skorch without time dependence
...
class NeuralNet_survival_notime:
    ...
    def proximal_operator(self, non_linear_weights, linear_weights, lambda1):
        ...
        v2 = self.gl_operator(non_linear_weights, l * lambda1 * (1-self.alpha)).astype('float32')
            ...
            if self.lambda1 != 0:
                with torch.no_grad():
                    for i in range(len(self.module_.linear)):
                        ...
                        prox_op = self.proximal_operator(non_linear_weights, linear_weights, self.lambda1 * self.lr)
                        j = 0
                        for param in self.module_.non_linear[i].parameters():
                            shape1 = param.shape
