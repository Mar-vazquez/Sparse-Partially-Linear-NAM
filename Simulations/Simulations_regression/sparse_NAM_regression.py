...
def sparse_NAM_fit_regression(X_train, y_train, X_test, y_test, seed, rest, seed2):
    ...
    for j in seed2:
        ...
        wrong_classif_reg, classif_reg = wrong_classif_regression(rest[-3], final_model_res, rest)
...
for i in range(len(seq_names)):
    # Read the data
    name_train = "results_file/n_700/train_" + seq_names[i] + '.csv'
    name_test = "results_file/n_700/test_" + seq_names[i] + '.csv'
    ...
    # Read the info
    name_info = "results_file/n_700/res_" + seq_names[i]
    f=open(name_info, "rb")
    b=pickle.load(f)
    f.close()
...
sparse_results_700 = [mse_sparse_NAM, wrong_sparse_NAM]
f=open( "results_file/n_700/sparse_NAM_res_700", "wb")
pickle.dump(sparse_results_700,f)
f.close()    
